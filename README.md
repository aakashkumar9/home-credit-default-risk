...
| 8 | Docker, CI, data validation, monitoring/retraining notes | done |
...
src/home_credit/       Installable package (pip install -e .)
  ...
  validation/            pandera schema for the mart_applicant_features contract
  serving/               FastAPI scoring app (api.py)
dashboard/               Streamlit dashboard (app.py)
warehouse/               dbt project: staging -> intermediate -> marts (done)
scripts/                 build_warehouse.sh (ingest -> dbt build -> dbt docs generate -> mart validation)
...
Dockerfile               Serving image (FastAPI app only)
.github/workflows/ci.yml Lint -> dbt build -> mart validation -> pytest
```
...
## Productionisation (done)

**Data validation (`src/home_credit/validation/schemas.py`, pandera).**
`MART_SCHEMA` validates the mart_applicant_features contract at the
warehouse/Python boundary - the point where a broken dbt model or raw-data
drift would otherwise surface as a confusing failure three modules
downstream instead of a clear column-level error right here. Not an
exhaustive per-column schema: the mart has ~130 mostly-float feature
columns that are legitimately nullable (no history to average over - see
the mart SQL's own comment on this), so those aren't individually
constrained. What IS checked: `sk_id_curr` (unique, non-null, >= 0),
`target` (0/1 or null - null is expected for the scoring set), the
`has_*_history` flags (always a non-null bool - a fixed regex over every
column matching that name, since each is generated by the identical SQL
expression), and the explicit list of `coalesce(x, 0)` count columns
(always non-negative, never null - **not** every column ending in
`_count`: `bureau_credit_types_count`, for one, is deliberately left NULL
with no bureau history, so it's excluded by name rather than swept in by a
looser regex). Runs via `make validate`, at the end of
`scripts/build_warehouse.sh` (right after `dbt build`), and again at the
start of every `make train` - the same contract is checked at both
production points a mart gets consumed from.

**CI (`.github/workflows/ci.yml`).** Lint (`ruff check` + `black --check`)
-> synthetic-fixture ingestion -> `dbt build` (schema + data tests) -> mart
validation -> the full `pytest` suite, on every push/PR against `main`.

**Docker (`Dockerfile`).** Packages the FastAPI serving app only - not
dbt/training/the dashboard, which are dev-time tooling a deployed scoring
service doesn't need at runtime. Models and the DuckDB warehouse are
intentionally **not** baked into the image: both are gitignored, per-run
artifacts (`make train`, `make dbt-build`), so they're bind-mounted in at
`docker run` time instead (`make docker-run`) - the image is the code, the
mount is the data.

**Monitoring & retraining (not implemented - notes only).** This is a
single-snapshot Kaggle competition dataset, so there's no live traffic to
monitor against; a real deployment on top of this pipeline would need:
population stability index (or a similar drift metric) on the mart's
feature distributions between training and current scoring data, tracked
per-feature so a shifting input (e.g. `EXT_SOURCE_*`, the strongest
predictors here) is caught before it silently degrades the model; a
calibration check on realized outcomes once ground truth is available,
since calibration drifts independently of ranking ability; and a
retraining trigger on a schedule or on drift-threshold breach, re-running
`make dbt-build && make train` and comparing the new champion's holdout
PR-AUC/calibration against the currently-served model before promoting it
- never promoting on CV metrics alone, the same held-out-evaluation
discipline `train.py` already uses for champion selection.
...
make dbt-build     # load raw CSVs -> dbt build (staging -> intermediate -> marts) -> dbt docs generate -> validate
...
make validate      # pandera check of the built mart, standalone (also runs inside dbt-build and train)
make docker-build && make docker-run   # build + run the FastAPI image (mounts models/ and data/)
```

Run `make` with no target (or open the `Makefile`) for the full command
list - every target above is live.
...
This is exactly what CI (`.github/workflows/ci.yml`) does today: lint ->
ingest -> `dbt build` -> mart validation -> the full `pytest` suite against
the resulting warehouse.
