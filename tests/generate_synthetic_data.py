...
def generate(out_dir: Path, n_applicants: int = 500, seed: int = 42):
    ...
    doc_cols = {
        f"FLAG_DOCUMENT_{i}": rng.binomial(1, 0.02, n_applicants) for i in [2, *range(3, 22)]
    }
    building_cols = {
    ...
    def ext_source():
        return np.where(rng.random(n_applicants) < 0.15, np.nan, rng.random(n_applicants))
    ...
    application = pd.DataFrame(
        {
            ...
            "NAME_CONTRACT_TYPE": rng.choice(
                ["Cash loans", "Revolving loans"], n_applicants, p=[0.9, 0.1]
            ),
            ...
            "NAME_TYPE_SUITE": rng.choice(
                ["Unaccompanied", "Family", "Spouse, partner"], n_applicants
            ),
            "NAME_INCOME_TYPE": rng.choice(
                ["Working", "Commercial associate", "Pensioner", "State servant"], n_applicants
            ),
            ...
            "NAME_FAMILY_STATUS": rng.choice(
                ["Married", "Single / not married", "Civil marriage", "Widow"], n_applicants
            ),
            "NAME_HOUSING_TYPE": rng.choice(
                ["House / apartment", "With parents", "Rented apartment"], n_applicants
            ),
            ...
            "OWN_CAR_AGE": np.where(
                rng.random(n_applicants) < 0.6, np.nan, rng.integers(0, 30, n_applicants)
            ),
            "FLAG_MOBIL": 1,
            ...
            "OCCUPATION_TYPE": rng.choice(
                ["Laborers", "Sales staff", "Core staff", "Managers", None], n_applicants
            ),
            ...
            "WEEKDAY_APPR_PROCESS_START": rng.choice(
                ["MONDAY", "TUESDAY", "WEDNESDAY", "THURSDAY", "FRIDAY"], n_applicants
            ),
            ...
            "ORGANIZATION_TYPE": rng.choice(
                ["Business Entity Type 3", "Self-employed", "Government", "XNA"], n_applicants
            ),
            ...
            "WALLSMATERIAL_MODE": rng.choice(
                ["Panel", "Block", "Stone, brick", None], n_applicants
            ),
            ...
            bureau_rows.append(
                {
                    ...
                    "DAYS_ENDDATE_FACT": (
                        days_credit + int(rng.integers(30, 2000)) if rng.random() < 0.5 else np.nan
                    ),
                    "DAYS_CREDIT_UPDATE": -int(rng.integers(0, 365)),
                    "CNT_CREDIT_PROLONG": int(rng.poisson(0.05)),
                    "AMT_CREDIT_MAX_OVERDUE": (
                        float(rng.exponential(500)) if rng.random() < 0.3 else np.nan
                    ),
                    "AMT_CREDIT_SUM": amt_credit_sum,
                    "AMT_CREDIT_SUM_DEBT": amt_credit_sum * rng.uniform(0, 0.9),
                    "AMT_CREDIT_SUM_LIMIT": (
                        float(rng.exponential(1000)) if rng.random() < 0.3 else np.nan
                    ),
                    "AMT_CREDIT_SUM_OVERDUE": (
                        float(rng.exponential(50)) if rng.random() < 0.05 else 0.0
                    ),
            ...
            prev_rows.append(
                {
                    ...
                    "CODE_REJECT_REASON": (
                        ...
                        else rng.choice(["HC", "LIMIT", "SCO"])
                    ),
                    "NAME_CLIENT_TYPE": rng.choice(
                        ["New", "Repeater", "Refreshed"], p=[0.2, 0.7, 0.1]
                    ),
                    ...
                    "AMT_CREDIT": amt_credit,
                    "AMT_DOWN_PAYMENT": (
                        amt_application * rng.uniform(0, 0.2) if not is_cash else np.nan
                    ),
                    ...
                    "CNT_PAYMENT": (
                        float(rng.integers(6, 36)) if contract_status == "Approved" else np.nan
                    ),
                    "DAYS_DECISION": days_decision,
                    "DAYS_FIRST_DRAWING": 365243,
                    "DAYS_FIRST_DUE": (
                        days_decision + 10 if contract_status == "Approved" else 365243
                    ),
                    "DAYS_LAST_DUE_1ST_VERSION": (
                        days_decision + 400 if contract_status == "Approved" else 365243
                    ),
                    "DAYS_LAST_DUE": (
                        days_decision + 380 if contract_status == "Approved" else 365243
                    ),
                    "DAYS_TERMINATION": (
                        days_decision + 390 if contract_status == "Approved" else 365243
                    ),
                    "FLAG_LAST_APPL_PER_CONTRACT": "Y",
                    "NFLAG_LAST_APPL_IN_DAY": 1,
                    "NFLAG_INSURED_ON_APPROVAL": (
                        rng.binomial(1, 0.3) if contract_status == "Approved" else np.nan
                    ),
                    ...
                    pos_rows.append(
                        {
                            ...
                            "NAME_CONTRACT_STATUS": (
                                "Active"
                                if m > 0
                                else rng.choice(["Active", "Completed"], p=[0.8, 0.2])
                            ),
